 ...
 * Storing intermediate result into files and put everything in a loop
 * But get summary from index tree
 */
public class BisectingKMeansV4 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (inputFile == null || k == null || iterations == null || dimension == null || outputDir == null) {
        ...
        File tempDir = new File(outputDir + "/tmp");
        if (!tempDir.exists()) {
            tempDir.mkdir();
        }
        ...
        while (loop < k) {
            ...
            if (loop == 1) {
                ...
                centerSummary = env.readTextFile(indexDir.getAbsolutePath())
                        .map(new MapFunction<String, ClusterCenter>() {
                            @Override
                            public ClusterCenter map(String s) throws Exception {
                                String[] cols = s.split(Constants.IN_DELIMITER);
                ...
                dataPoints = env.readTextFile(tempDir.getAbsolutePath())
...
//                                System.out.println("Read: " + Integer.parseInt(cols[0]) + " - " + point.toString());
            ...
            DataSet<IndexedPoint> result = divideData(dividableData, dividableLeafNode, indexDir.getAbsolutePath(), centerSummary, iterations, threshold)
                    .union(undividableData);

            // write to temp folder
            result.map(new MapFunction<IndexedPoint, Tuple2<Integer, String>>() {
                @Override
                public Tuple2<Integer, String> map(IndexedPoint indexedPoint) throws Exception {
                    ...
                    .writeAsCsv(tempDir.getAbsolutePath(), "\n", Constants.IN_DELIMITER, FileSystem.WriteMode.OVERWRITE);

            loop++;
            env.execute("Bisecting Kmeans V4 Clustering");
        }
    }
...
//                        System.out.println("summarize indexed point " + indexedPoint);
...
//                        System.out.println("summarize " + center);
                        return center;
                    }
                });
        return centers;
...
//                System.out.println("Center in getDividableLeafNode: " + maxCenter.toString());
    ...
    public static DataSet<IndexedPoint> divideData(DataSet<IndexedPoint> data, final ClusterCenter center, String indexDir, DataSet<ClusterCenter> centerSummary, int maxIterations, final float threshold) throws Exception {
